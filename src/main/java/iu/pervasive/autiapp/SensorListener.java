...
import iu.pervasive.autiapp.ui.Activity_Main;
import iu.pervasive.autiapp.util.Logger;
import iu.pervasive.autiapp.util.Util;
import iu.pervasive.autiapp.widget.WidgetUpdateService;
...
public class SensorListener extends Service implements SensorEventListener {
        ...
        if (steps > lastSaveSteps + SAVE_OFFSET_STEPS ||
            ...
            if (db.getSteps(Util.getToday()) == Integer.MIN_VALUE) {
                int pauseDifference = steps -
                        getSharedPreferences("autiapp", Context.MODE_PRIVATE)
                                .getInt("pauseCount", steps);
                db.insertNewDay(Util.getToday(), steps - pauseDifference);
                if (pauseDifference > 0) {
                    // update pauseCount for the new day
                    getSharedPreferences("autiapp", Context.MODE_PRIVATE).edit()
    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        if (intent != null && ACTION_PAUSE.equals(intent.getStringExtra("action"))) {
            ...
            SharedPreferences prefs = getSharedPreferences("autiapp", Context.MODE_PRIVATE);
